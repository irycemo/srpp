...
class Gravamen extends Model implements Auditable
{
    ...
    public function deudoresUnicos(){
        return $this->hasMany(Deudor::class)->where('tipo', 'I-DEUDOR ÚNICO');
    }

    public function garantesHipotecarios(){
        return $this->hasMany(Deudor::class)->where('tipo', 'D-GARANTE(S) HIPOTECARIO(S)');
    }

    public function parteAlicuota(){
        return $this->hasMany(Deudor::class)->where('tipo', 'P-PARTE ALICUOTA');
    }

    public function garantesCoopropiedad(){
        return $this->hasMany(Deudor::class)->where('tipo', 'G-GARANTES EN COOPROPIEDAD');
    }

    public function fianza(){
        return $this->hasMany(Deudor::class)->where('tipo', 'F-FIANZA');
    }
